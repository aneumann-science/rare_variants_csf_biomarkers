...
genes_adni <- list.files(path="../adni_emif_genotypes/adni/lof/plink", pattern="*.raw", full.names=FALSE, recursive=FALSE)
genes_emif <- list.files(path="../adni_emif_genotypes/emif/lof/plink", pattern="*.raw", full.names=FALSE, recursive=FALSE)
...
save(mmse_meta_results.data, file = "results/lof/nodiag/mmse_meta_results.Rdata")
...
save(tau_pc_meta_results.data, file = "results/lof/nodiag/tau_pc_meta_results.Rdata")
...
save(ab_pc_meta_results.data, file = "results/lof/nodiag/ab_pc_meta_results.Rdata")
...
save(nfl_pc_meta_results.data, file = "results/lof/nodiag/nfl_pc_meta_results.Rdata")
...
save(ykl40_pc_meta_results.data, file = "results/lof/nodiag/ykl40_pc_meta_results.Rdata")
...
save(ng_pc_meta_results.data, file = "results/lof/nodiag/ng_pc_meta_results.Rdata")

# Annotation for effective number of carriers
source("skat_o_meta_lof_annot.R")
lof.list <- pbmclapply(genes_adni, function(gene) {
  skat_o_meta_lof_annot(pca_adni.data,"RC5",pca_emif.data,"RC5",csf_cov_adni.data,csf_cov_emif.data,gene)
}, mc.cores = 12)

lof_annot.data <- do.call(rbind, lof.list)
save(lof_annot.data, file = "results/lof/nodiag/lof_annot.Rdata")